...
app.post('/add-entry', function(request, response) {
    db.collection('ExpensesData').insertOne(request.body).then(function() { // expensedata(collection name)
...
/*
app.patch('/update-entry/:id',function(request,response) { // patch put same (-update)-to receive params (:id)(example)
    //need body and id
    console.log(request.params)//params-/ after -conetnt,(localhost:8000/update-entry/id..)
})
...
app.patch('/update-entry/:id', function(request, response) {
    if(ObjectId.isValid(request.params.id)) {
        db.collection('ExpensesData').updateOne(
            ...
            { $set : request.body } // The data to be updated // mongodb -update method
        ).then(function() {
            response.status(200).json({
...
})//local connection terminated and connect to atlas
//mongo db is connect in local server ,now cloud serevr-atlas
*/
